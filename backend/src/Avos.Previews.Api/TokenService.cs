...
/// <summary>JWT issuing, mirroring avos-vault's TokenService: HS256 on Jwt:Key,
/// access 15 minutes, refresh 7 days (hashed at rest, rotated on refresh). sessionId is the
/// RefreshToken row this access token belongs to, carried as a "sid" claim — what
/// SessionRevocationMiddleware checks so a revoked/rotated session dies within one request
/// rather than whenever the access token would have expired on its own.</summary>
public class TokenService(IConfiguration config)
{
    ...
    public (string Token, DateTimeOffset ExpiresAt) CreateAccessToken(
        string userId, string email, string fullName, Guid? sessionId = null)
    {
        ...
        if (sessionId is { } sid) claims.Add(new Claim("sid", sid.ToString()));
